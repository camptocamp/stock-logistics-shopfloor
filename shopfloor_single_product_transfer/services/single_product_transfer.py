...
class ShopfloorSingleProductTransfer(Component):
    ...
    * scan the destination location
    ...
    def _scan_location__default_quant_domain(self, location):
    ...
    def _scan_location__check_stock(self, location):
        ...
        quants = self.env["stock.quant"].search(
            self._scan_location__default_quant_domain(location), limit=1
        )
        if not quants:
    ...
    def _scan_location__check_stock_packages(self, location):
        ...
        domain = AND(
            [
                self._scan_location__default_quant_domain(location),
                [("package_id", "=", False)],
            ]
        )
